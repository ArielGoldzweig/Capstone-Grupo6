...
for d in lista_drivers:
    folium.CircleMarker(d.origen, color='black', radius=4, fill=True).add_to(m) 
    for e in d.ruta:
        folium.CircleMarker(e, color='blue', radius=2, fill=True).add_to(m)
        folium.PolyLine([d.origen, e], color='red', weight=3, opacity=1).add_to(m)


m.save('simulation/maps/Cluster.html')